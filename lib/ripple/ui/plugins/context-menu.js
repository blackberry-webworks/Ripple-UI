    ...
    if (menuCurrentState === state.PEEK) {
        ...
        if (numItems > maxNumItemsInPeekMode) {
            ...
            if (moreIcon === null) {
                ...
                moreIcon.src = jQuery("#extension-url").text() + 'images/webworks/bb10/ActionOverflowMenu.png';
...
//Changed for Ripple because this function did not take outer scrolling into the equation and is not as performance focused
function getMenuItemAtPosition(evt) {
    var touchedElement = document.elementFromPoint(evt.clientX, evt.clientY),
        index;

    //Three cases we care about
        //1. We have selected a contextMenuItem
        //2. We have selected the img (a child of the contextMenuItem)
        //3. We have selected something outside

    if (!touchedElement.classList.contains("contextmenuItem")) {
        if (touchedElement.tagName.toUpperCase() === "IMG" && touchedElement.parentElement.classList.contains("contextmenuItem")) {
    ...
    //elements should be an array, but in some rare cases it somehow isn't .....
    if (!Array.isArray(elements)) {
        elements = Array.prototype.slice.call(elements, 0);
    }

    index = elements.indexOf(touchedElement);

    return index;
}

function menuTouchMoveHandler(evt) {
    var currentPeekIndex,
        i;

    evt.stopPropagation();

    //Code changed for Ripple, no longer using complicated math
    currentPeekIndex = getMenuItemAtPosition(evt);
...
function menuItemTouchStartHandler(evt) {
    ...
    previousPeekIndex = currentPeekIndex = elements.indexOf(evt.currentTarget);
}
...
function menuItemTouchEndHandler(evt) {
    ...
    if (currentPeekIndex !== -1) {
        ...
        event.trigger('contextmenu.executeMenuAction', [element.attributes.actionId.value]);
